...
static KmerFilterCmdLine parse_cmdline(int argc, char *argv[], int unit_size)
{
    ...
    while ((opt = getopt_long(argc, argv,"1:2:b:f:hi:k:n:o:p:q:r:s:t:O:", long_options, &longopt_index)) > 0){
        switch (opt) {
            ...
            case 't':
                ...
                if ( cmd_line.threashold  < 0 ||  cmd_line.threashold  > 1) {
                    errx(1, "[-t | --threashold] option requires a number float between 0 and 1");
                }
