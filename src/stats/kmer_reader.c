...
static int file_reader_wrapper(KmerFileReaderInnerArgs * fria){
    int length = 0;
    int offset;
    assert(fria->fp != NULL);
    switch(fria->format){
...
boolean check_binary_signature_kmers(FILE * fp, uint32_t kmer_size, uint32_t binary_version, uint32_t* number_of_colours_in_binary, uint32_t* mean_read_len, uint64_t* total_seq)
{
    size_t read;
    char magic_number[6];
    boolean ret = false;
